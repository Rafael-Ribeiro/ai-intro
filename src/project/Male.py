...
def leftActivator(vehicle, leftProximitySensor, rightProximitySensor, leftLightSensor, rightSmellSensor, leftSoundSensor):
	...
	return VELOCITY*(a - 2*leftLightSensor*(1-vehicle.sexuality) + 2*rightSmellSensor*vehicle.sexuality + 0.5*leftSoundSensor)
...
def rightActivator(vehicle, leftProximitySensor, rightProximitySensor, rightLightSensor, leftSmellSensor, rightSoundSensor):
	...
	return VELOCITY*(b - 2*rightLightSensor*(1-vehicle.sexuality) +3*leftSmellSensor*vehicle.sexuality + 0.5*rightSoundSensor)
...
class MaleVehicle(breve.BraitenbergVehicle):
	def __init__(self):
		...
		self.handleCollisions("FemaleVehicle", "reproduce")
		...
		# Proximity
		self.leftProximitySensor  = breve.createInstances(ProximitySensor, 1, 'leftProximitySensor', math.pi/3, [SphereStationary, MaleVehicle], DISTANCE_BIAS)
		self.rightProximitySensor  = breve.createInstances(ProximitySensor, 1, 'rightProximitySensor', math.pi/3, [SphereStationary, MaleVehicle], DISTANCE_BIAS)
	...
	def updateSexuality(self):
		...
		self.cry.setIntensity(1-self.maturity)
