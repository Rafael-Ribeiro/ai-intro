...
class Control( breve.Abstract ):
	...
	def iterate( self ):
		if self.watchObject:
			location = self.watchObject.getLocation()
			if location.__class__.__name__ == "vector":
				self.setCameraTarget( location )
			else:
				self.watch(None)

		if ( not self.frozen ):
			if ( breve.breveInternalFunctionFinder.worldStep( self, self.simTime, self.simStep ) == -1 ):
