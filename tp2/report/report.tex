...
\cleardoublepage
\subsection{Representation}
\indent Como é possível observar pelo gráfico (ou constatar nas tabelas da secção \ref{sec:fitness_by_representation}), a representação fixa converge mais rapidamente para o óptimo global
...
\begin{center}
	\begin{tabular}{ccc|c|}
																				\cline{3-4}
		\multicolumn{2}{c}{\multirow{2}{*}{}}									& \multicolumn{2}{| c |}{Initial Points}\\
																				\cline{3-4}
																	&			& \multicolumn{1}{|c|}{P$_{1}$}	& \multicolumn{1}{c|}{P$_{2}$}	\\
		\hline
		\multicolumn{1}{|c|}{\multirow{2}{*}{Number of Points}} 	& 15		& \multicolumn{1}{|c|}{NA}	& NA							\\
																	\cline{2-4}
		\multicolumn{1}{|c|}{}										& 30 		& \multicolumn{1}{|c|}{NA}	& NA							\\
		\hline
	\end{tabular}
	\captionof{table}{Representation break points}
	\label{tab:representation_break_points}
\end{center}

\cleardoublepage

\subsection{Selection}
\indent TODO (secção \ref{sec:fitness_by_selection}) % FIXME % TODO
