...
\section{Experiments}
\indent \indent Os resultados obtidos com o algoritmo genético dependem intrinsecamente dos parâmetros com os quais é executado
(representação, modo de selecção, percentagem de elitismo, ...). Assim, para uma correcta análise do seu comportamento
torna-se necessário variá-los e analisar as variadas combinações. Como tal, cada parâmetro foi variado 2/3 vezes.
Enumeram-se de seguida as variações.

\begin{description}
	\item [Points set] Foram utilizados dois conjuntos de pontos (inicial e final) para averiguar a correcta convergência em diferentes casos.
		Estes são P$_{init} = (0.0, 3.0)$, P$_{final} = (4.0, 2.0)$ e P$_{init} = (0.0, 3.0)$, P$_{final} = (4.0, 2.8)$.
	\item [Representation] Foram testados os tipos de representação já mencionados em \ref{subsec:representation}.
	\item [Selection] Foram testados os tipos de selecção já mencionados em \ref{subsec:selection}. No entanto, o elitismo foi variado em simultâneo com
		as representações Fixa e Dinâmica, uma vez que são aplicadas em fases diferentes do algoritmo.
	\begin{description}
		\item [Elitismo] ...
	\end{description}
\end{description}
